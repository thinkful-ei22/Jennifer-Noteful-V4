...
describe('Noteful API - Notes', function () {
  ...
  describe('GET /api/notes', function () {
    ...
    it('should return a list with the correct right fields', function () {
      const dbPromise = Note.find({userId: user.id}).sort({ updatedAt: 'desc' });
      const apiPromise = chai.request(app)
        .get('/api/notes')
        .set('Authorization', `Bearer ${token}`);
    ...
    it.only('should return an error when a tags `id` is not valid ', function () {//currently not validating tag ids in notes routes!
      ...
      return chai.request(app)
        ...
        .then(res => {
          ...
          expect(res).to.be.json;
          expect(res.body).to.be.a('object');
          expect(res.body.message).to.equal('The `tagId` is not valid');
        });
    });
  ...
  describe('PUT /api/notes/:id', function () {
    ...
    it('should return an error when a tags `id` is not valid ', function () {//we don't validate tag ids in the route!
      const updateItem = {
        tags: ['NOT-A-VALID-ID']
      };
      return Note.findOne({userId: user.id})
